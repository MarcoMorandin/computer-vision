...
class ViTPoseEstimator:
    ...
    def __init__(
        ...
        config : Any | None
            Configuration with detector and ViTPose settings.
        ...
        self.vit_model = VitPoseForPoseEstimation.from_pretrained(model_name).to(
            self.device
        )
        self.vit_model.eval()
    ...
    def run_pose_estimation_on_video(
        ...
        for frame_idx in tqdm(
            range(total_frames), desc="Running ViTPose Estimation", unit="frame"
        ):
            _, frame = cap.read()
            ...
            keypoints = self._process_frame(
                frame, img_id, category_id, confidence_threshold
            )
    ...
    def _process_frame(
        self, frame, img_id: int, category_id: int, confidence_threshold: float
    ) -> Optional[List[float]]:
        ...
        inputs = self.processor(pil_img, boxes=[boxes_xywh], return_tensors="pt").to(
            self.device
        )
        ...
        pose_results_list = self.processor.post_process_pose_estimation(
