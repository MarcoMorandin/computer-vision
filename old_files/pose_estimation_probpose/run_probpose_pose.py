"""Dataset-wide ProbPose estimation pipeline.

This mirrors the structure of `pose_estimation_vit/run_vit_pose.py` but uses:
  1. A YOLO (detection only) model to find person bounding boxes.
  2. A ProbPose (MMPose) top-down model to estimate COCO keypoints inside those boxes.

It loads an input COCO json (e.g. the rectified dataset), prunes foot/toe keypoints
from the category (consistent with the ViT script), rebuilds annotations for every
image, and writes a new predictions COCO json.

Prerequisites:
  - ProbPose repo (MMPose fork) already installed (user stated it's installed).
  - Checkpoint file: `pose_estimation_probpose/ProbPose.pth` (adjust path if different).
  - Config file: `pose_estimation_probpose/ProbPose/configs/body_2d_keypoint/topdown_probmap/coco/td-pm_ProbPose-small_8xb64-210e_coco-256x192.py`
    (You can switch to another config / checkpoint pair if desired.)

Usage example (from repository root):      
python pose_estimation_probpose/run_probpose_pose.py \                                 ─╯
  --input-image-dir rectification/output/dataset/train \
  --input-json rectification/output/dataset/train/_annotations.coco.json \
  --output-json pose_estimation_probpose/output/predictions_coco_probpose.json \
  --config pose_estimation_probpose/ProbPose/configs/body_2d_keypoint/topdown_probmap/coco/td-pm_ProbPose-small_8xb64-210e_coco-256x192.py \
  --checkpoint pose_estimation_probpose/ProbPose.pth \
  --detector-model '/Users/marcomorandin/Desktop/ComputerVision/computer-vision/pose_estimation_vit/yolo11l.pt'\
  --det-conf 0.6
"""
